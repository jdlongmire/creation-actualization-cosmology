...
It is **not a novel-fact prediction about nature**. It forbids a pattern in how models evolve; it does not forbid an observation of the world. CAC still owes one of those, and [OPN-011](../traceability/claims/OPN-011.yaml) — the missing likelihood function — remains the programme's sharpest open problem. PRD-001 moving the ledger to one prediction should not be read as OPN-011 being answered, and the [appraisal log](../3-prediction/appraisal.md) records the distinction explicitly so that the count cannot quietly stand in for the kind.
