...
**Status: CAC v0.1 — a position paper and a programme scaffold, not a cosmological model.** The programme's own [appraisal](3-prediction/appraisal.md) records it as **empirically unappraised, with two theoretically progressive steps taken**: 2 predictions ([PRD-001](traceability/claims/PRD-001.yaml), [PRD-002](traceability/claims/PRD-002.yaml), both protocols unexecuted), 5 accommodations, none of which forbids anything, and **0 novel-fact predictions about nature**. Both predictions forbid patterns in a rival's models — how they evolve, and where their information sits — which are legitimate Lakatosian objects and a weaker species than the programme still owes. The ledger is generated on every build, not asserted, and it is the honest starting point rather than a defect to be managed.
...
| **[2-theory/](2-theory/)** | **Protective belt** — the adjustable work | [exegesis](2-theory/00-exegesis/) · [actualization](2-theory/01-actualization/) · [metric & redshift](2-theory/02-metric/) · [entropy](2-theory/03-entropy/) · [dark sector](2-theory/04-dark-sector/) · [specification](2-theory/05-specification/) · [open problems](2-theory/open-problems.md) |
| **[3-prediction/](3-prediction/)** | **Severe tests** — claims that forbid something | [discriminators](3-prediction/discriminators.md) *(2 claims, 0 of them about nature — this is the critical path)* · [appraisal](3-prediction/appraisal.md) |
...
**Point 6 also fixes the scope of the disagreement** ([EPI-003](traceability/claims/EPI-003.yaml)). The branch from design-assuming natural philosophy to what is now called methodological naturalism was *two* transitions, and CAC contests only the second. Final causes leaving proximate explanation (Bacon 1620, Descartes 1644) is accepted by point 6 by construction. Uniformitarian historical inference — Hutton 1788, Lyell 1830–33, explain the past by causes now in operation — is the commitment CAC contests, because it concerns historical boundary conditions rather than operational method. The rival account is stated at full strength in the claim: that branch was *earned*, and diluvial geology was abandoned by its own leading practitioner on the evidence, Buckland dropping the Flood correlation in his own 1836 Bridgewater Treatise. Reopening the branch point owes an argument that he was wrong on the evidence then available, and CAC has not supplied one.
...
Two disciplines ship with it. **The test is void, not confirmed, if operational accuracy stops improving** — stagnation in the rival programme can never be counted as CAC's success. And **methodological reciprocity**: CAC is evaluated by the same instrument, mechanized at [`0-program-methods/ops/reciprocity_check.py`](0-program-methods/ops/reciprocity_check.py), which currently reports 21.5 claims per claim that forbids anything. That reading is unflattering and it is printed on every build for the same reason the accommodation ledger is.

## Initial-condition burden — PRD-002

[PRD-002](traceability/claims/PRD-002.yaml) sharpens PRD-001's asymmetry into a structural claim with a named opponent: explanatory burden for historical origins migrates **from the dynamics into the initial conditions**. Of the total information needed to specify the observed universe, the proportion residing in the *state* — primordial spectrum shape, isocurvature content, primordial non-Gaussianity, boundary assumptions — will not decline relative to the proportion residing in the *laws*.

The rival prediction is explicit and opposite. The inflationary programme exists in substantial part to derive flatness and homogeneity from generic initial data, which is information migrating *out* of the state and into the dynamics. If that programme succeeds — including on the measure problem and the low-entropy past — PRD-002 is refuted, and refuted in a way that counts as positive evidence against CAC's account of where information originates rather than as a mere absence.

**It may already be losing.** Planck constrained the running of the spectral index and primordial non-Gaussianity to values consistent with the simplest single-field inflationary initial conditions. That is the state side *not* growing. The claim is registered anyway, on the view that a prediction which could not already be losing was not worth registering.

The rationale sits in the belt at [`2-theory/05-specification/`](2-theory/05-specification/specification-and-actualization.md), which was drafted as a second prediction about investigator-supplied specification and **demoted on review**: that version is confirmed by construction, has no contrast class, and had an unsatisfiable falsifier. The demotion is in the [appraisal log](3-prediction/appraisal.md) rather than done quietly.
